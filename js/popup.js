	...
	_popup.prototype.load = function(){
		...
		this.btn_create.addEventListener('click',function(){
			...
			if( this.btn_create.classList.contains('loading') ){return false;}
			this.create();
		}.bind(this));
		this.btn_destroy.addEventListener('click',function(){
			/* If the button is loading, then we cant do this twice */
			if( this.btn_destroy.classList.contains('loading') ){return false;}
			this.destroy();
		}.bind(this));
	...
	_popup.prototype.exists = function(){
		return new Promise(function (resolve, reject) {
			var message = {'command':'mail.exists'};
			chrome.runtime.sendMessage(message,function(resp){
				if( !resp ){
					reject({'error':'NOT_EXISTS'});
					return false;
				}
	...
	_popup.prototype.create = function(){
		return new Promise(function (resolve, reject) {
			...
			var message = {'command':'mail.create'};
			chrome.runtime.sendMessage(message,function(resp){
				if( resp.error ){
					console.log(resp);
					reject(resp);
...
//FIXME: devolver botón a su estado normal
	...
	_popup.prototype.destroy = function(){
		return new Promise(function (resolve, reject) {
			...
			var message = {'command':'mail.destroy'};
			chrome.runtime.sendMessage(message,function(resp){
				resolve(resp);
				this.welcome();
	...
	_popup.prototype.inbox = function(){
		...
		var message = {'command':'mail.inbox'};
		chrome.runtime.sendMessage(message,function(resp){
